import {Component, EventEmitter, OnInit, Output} from '@angular/core';
import {AngularMobilenumberCountrycodeDropdownService} from './angular-mobilenumber-countrycode-dropdown.service';

@Component({
  selector: 'lib-angular-mobilenumber-countrycode-dropdown',
  templateUrl: './angular-mobilenumber-countrycode-dropdown.component.html',
  providers: [AngularMobilenumberCountrycodeDropdownService]
})
export class AngularMobilenumberCountrycodeDropdownComponent implements OnInit {

  @Output() countrySelectHandler: EventEmitter<any> = new EventEmitter();
  countryCodeList: any = [
    {
      country: 'Afghanistan',
      code: '+93',
      value: '93'
    },
    {
      country: 'Åland Islands',
      code: '+358 18',
      value: '35818'
    },
    {
      country: 'Albania',
      code: '+355',
      value: '355'
    },
    {
      country: 'Algeria',
      code: '+213',
      value: '213'
    },
    {
      country: 'American Samoa',
      code: '+1 684',
      value: '1684'
    },
    {
      country: 'Andorra',
      code: '+376',
      value: '376'
    },
    {
      country: 'Angola',
      code: '+244',
      value: '244'
    },
    {
      country: 'Anguilla',
      code: '+1 264',
      value: '1264'
    },
    {
      country: 'Antigua and Barbuda',
      code: '+1 268',
      value: '1268'
    },
    {
      country: 'Argentina',
      code: '+54',
      value: '54'
    },
    {
      country: 'Armenia',
      code: '+374',
      value: '374'
    },
    {
      country: 'Aruba',
      code: '+297',
      value: '297'
    },
    {
      country: 'Ascension',
      code: '+247',
      value: '247'
    },
    {
      country: 'Australia',
      code: '+61',
      value: '61'
    },
    {
      country: 'Australian Antarctic Territory',
      code: '+672 1',
      value: '6721'
    },
    {
      country: 'Australian External Territories',
      code: '+672',
      value: '672'
    },
    {
      country: 'Austria',
      code: '+43',
      value: '43'
    },
    {
      country: 'Azerbaijan',
      code: '+994',
      value: '994'
    },
    {
      country: 'Bahamas',
      code: '+1 242',
      value: '1242'
    },
    {
      country: 'Bahrain',
      code: '+973',
      value: '973'
    },
    {
      country: 'Bangladesh',
      code: '+880',
      value: '880'
    },
    {
      country: 'Barbados',
      code: '+1 246',
      value: '1246'
    },
    {
      country: 'Barbuda',
      code: '+1 268',
      value: '1268'
    },
    {
      country: 'Belarus',
      code: '+375',
      value: '375'
    },
    {
      country: 'Belgium',
      code: '+32',
      value: '32'
    },
    {
      country: 'Belize',
      code: '+501',
      value: '501'
    },
    {
      country: 'Benin',
      code: '+229',
      value: '229'
    },
    {
      country: 'Bermuda',
      code: '+1 441',
      value: '1441'
    },
    {
      country: 'Bhutan',
      code: '+975',
      value: '975'
    },
    {
      country: 'Bolivia',
      code: '+591',
      value: '591'
    },
    {
      country: 'Bonaire',
      code: '+599 7',
      value: '5997'
    },
    {
      country: 'Bosnia and Herzegovina',
      code: '+387',
      value: '387'
    },
    {
      country: 'Botswana',
      code: '+267',
      value: '267'
    },
    {
      country: 'Brazil',
      code: '+55',
      value: '55'
    },
    {
      country: 'British Indian Ocean Territory',
      code: '+246',
      value: '246'
    },
    {
      country: 'British Virgin Islands',
      code: '+1 284',
      value: '1284'
    },
    {
      country: 'Brunei Darussalam',
      code: '+673',
      value: '673'
    },
    {
      country: 'Bulgaria',
      code: '+359',
      value: '359'
    },
    {
      country: 'Burkina Faso',
      code: '+226',
      value: '226'
    },
    {
      country: 'Burundi',
      code: '+257',
      value: '257'
    },
    {
      country: 'Cambodia',
      code: '+855',
      value: '855'
    },
    {
      country: 'Cameroon',
      code: '+237',
      value: '237'
    },
    {
      country: 'Canada',
      code: '+1',
      value: '1'
    },
    {
      country: 'Cape Verde',
      code: '+238',
      value: '238'
    },
    {
      country: 'Caribbean Netherlands',
      code: '+599 3',
      value: '5993'
    },
    {
      country: 'Cayman Islands',
      code: '+1 345',
      value: '1345'
    },
    {
      country: 'Central African Republic',
      code: '+236',
      value: '236'
    },
    {
      country: 'Chad',
      code: '+235',
      value: '235'
    },
    {
      country: 'Chatham Island, New Zealand',
      code: '+64',
      value: '64'
    },
    {
      country: 'Chile',
      code: '+56',
      value: '56'
    },
    {
      country: 'China',
      code: '+86',
      value: '86'
    },
    {
      country: 'Christmas Island',
      code: '+61 89164',
      value: '6189164'
    },
    {
      country: 'Cocos (Keeling) Islands',
      code: '+61 89162',
      value: '6189162'
    },
    {
      country: 'Colombia',
      code: '+57',
      value: '57'
    },
    {
      country: 'Comoros',
      code: '+269',
      value: '269'
    },
    {
      country: 'Congo',
      code: '+242',
      value: '242'
    },
    {
      country: 'Congo, Democratic Republic of the (Zaire)',
      code: '+243',
      value: '243'
    },
    {
      country: 'Cook Islands',
      code: '+682',
      value: '682'
    },
    {
      country: 'Costa Rica',
      code: '+506',
      value: '506'
    },
    {
      country: 'Croatia',
      code: '+385',
      value: '385'
    },
    {
      country: 'Cuba',
      code: '+53',
      value: '53'
    },
    {
      country: 'Curaçao',
      code: '+599 9',
      value: '5999'
    },
    {
      country: 'Cyprus',
      code: '+357',
      value: '357'
    },
    {
      country: 'Czech Republic',
      code: '+420',
      value: '420'
    },
    {
      country: 'Denmark',
      code: '+45',
      value: '45'
    },
    {
      country: 'Diego Garcia',
      code: '+246',
      value: '246'
    },
    {
      country: 'Djibouti',
      code: '+253',
      value: '253'
    },
    {
      country: 'Dominica',
      code: '+1 767',
      value: '1767'
    },
    {
      country: 'Dominican Republic',
      code: '+1 809',
      value: '1809'
    },
    {
      country: 'Dominican Republic',
      code: '+1 829',
      value: '1829'
    },
    {
      country: 'Dominican Republic',
      code: '+1 849',
      value: '1849'
    },
    {
      country: 'East Timor',
      code: '+670',
      value: '670'
    },
    {
      country: 'Easter Island',
      code: '+56',
      value: '56'
    },
    {
      country: 'Ecuador',
      code: '+593',
      value: '593'
    },
    {
      country: 'Egypt',
      code: '+20',
      value: '20'
    },
    {
      country: 'El Salvador',
      code: '+503',
      value: '503'
    },
    {
      country: 'Ellipso (Mobile Satellite service)',
      code: '+881 2',
      value: '8812'
    },
    {
      country: 'Ellipso (Mobile Satellite service)',
      code: '+881 3',
      value: '8813'
    },
    {
      country: 'EMSAT (Mobile Satellite service)',
      code: '+882 13',
      value: '88213'
    },
    {
      country: 'Equatorial Guinea',
      code: '+240',
      value: '240'
    },
    {
      country: 'Eritrea',
      code: '+291',
      value: '291'
    },
    {
      country: 'Estonia',
      code: '+372',
      value: '372'
    },
    {
      country: 'eSwatini',
      code: '+268',
      value: '268'
    },
    {
      country: 'Ethiopia',
      code: '+251',
      value: '251'
    },
    {
      country: 'Falkland Islands',
      code: '+500',
      value: '500'
    },
    {
      country: 'Faroe Islands',
      code: '+298',
      value: '298'
    },
    {
      country: 'Fiji',
      code: '+679',
      value: '679'
    },
    {
      country: 'Finland',
      code: '+358',
      value: '358'
    },
    {
      country: 'France',
      code: '+33',
      value: '33'
    },
    {
      country: 'French Antilles',
      code: '+596',
      value: '596'
    },
    {
      country: 'French Guiana',
      code: '+594',
      value: '594'
    },
    {
      country: 'French Polynesia',
      code: '+689',
      value: '689'
    },
    {
      country: 'Gabon',
      code: '+241',
      value: '241'
    },
    {
      country: 'Gambia',
      code: '+220',
      value: '220'
    },
    {
      country: 'Georgia',
      code: '+995',
      value: '995'
    },
    {
      country: 'Germany',
      code: '+49',
      value: '49'
    },
    {
      country: 'Ghana',
      code: '+233',
      value: '233'
    },
    {
      country: 'Gibraltar',
      code: '+350',
      value: '350'
    },
    {
      country: 'Global Mobile Satellite System (GMSS)',
      code: '+881',
      value: '881'
    },
    {
      country: 'Globalstar (Mobile Satellite Service)',
      code: '+881 8',
      value: '8818'
    },
    {
      country: 'Globalstar (Mobile Satellite Service)',
      code: '+881 9',
      value: '8819'
    },
    {
      country: 'Greece',
      code: '+30',
      value: '30'
    },
    {
      country: 'Greenland',
      code: '+299',
      value: '299'
    },
    {
      country: 'Grenada',
      code: '+1 473',
      value: '1473'
    },
    {
      country: 'Guadeloupe',
      code: '+590',
      value: '590'
    },
    {
      country: 'Guam',
      code: '+1 671',
      value: '1671'
    },
    {
      country: 'Guatemala',
      code: '+502',
      value: '502'
    },
    {
      country: 'Guernsey',
      code: '+44 1481',
      value: '441481'
    },
    {
      country: 'Guernsey',
      code: '+44 7781',
      value: '447781'
    },
    {
      country: 'Guernsey',
      code: '+44 7839',
      value: '447839'
    },
    {
      country: 'Guernsey',
      code: '+44 7911',
      value: '447911'
    },
    {
      country: 'Guinea',
      code: '+224',
      value: '224'
    },
    {
      country: 'Guinea-Bissau',
      code: '+245',
      value: '245'
    },
    {
      country: 'Guyana',
      code: '+592',
      value: '592'
    },
    {
      country: 'Haiti',
      code: '+509',
      value: '509'
    },
    {
      country: 'Honduras',
      code: '+504',
      value: '504'
    },
    {
      country: 'Hong Kong',
      code: '+852',
      value: '852'
    },
    {
      country: 'Hungary',
      code: '+36',
      value: '36'
    },
    {
      country: 'Iceland',
      code: '+354',
      value: '354'
    },
    {
      country: 'ICO Global (Mobile Satellite Service)',
      code: '+881 0',
      value: '8810'
    },
    {
      country: 'ICO Global (Mobile Satellite Service)',
      code: '+881 1',
      value: '8811'
    },
    {
      country: 'India',
      code: '+91',
      value: '91'
    },
    {
      country: 'Indonesia',
      code: '+62',
      value: '62'
    },
    {
      country: 'Inmarsat SNAC',
      code: '+870',
      value: '870'
    },
    {
      country: 'International Freephone Service (UIFN)',
      code: '+800',
      value: '800'
    },
    {
      country: 'International Networks',
      code: '+882',
      value: '882'
    },
    {
      country: 'International Networks',
      code: '+883',
      value: '883'
    },
    {
      country: 'International Premium Rate Service',
      code: '+979',
      value: '979'
    },
    {
      country: 'International Shared Cost Service (ISCS)',
      code: '+808',
      value: '808'
    },
    {
      country: 'Iran',
      code: '+98',
      value: '98'
    },
    {
      country: 'Iraq',
      code: '+964',
      value: '964'
    },
    {
      country: 'Ireland',
      code: '+353',
      value: '353'
    },
    {
      country: 'Iridium (Mobile Satellite service)',
      code: '+881 6',
      value: '8816'
    },
    {
      country: 'Iridium (Mobile Satellite service)',
      code: '+881 7',
      value: '8817'
    },
    {
      country: 'Isle of Man',
      code: '+44 1624',
      value: '441624'
    },
    {
      country: 'Isle of Man',
      code: '+44 7524',
      value: '+447524'
    },
    {
      country: 'Isle of Man',
      code: '+44 7624',
      value: '+447624'
    },
    {
      country: 'Isle of Man',
      code: '+44 7924',
      value: '447924'
    },
    {
      country: 'Israel',
      code: '+972',
      value: '972'
    },
    {
      country: 'Italy',
      code: '+39',
      value: '39'
    },
    {
      country: 'Ivory Coast',
      code: '+225',
      value: '225'
    },
    {
      country: 'Jamaica',
      code: '+1 876',
      value: '1876'
    },
    {
      country: 'Jan Mayen',
      code: '+47 79',
      value: '4779'
    },
    {
      country: 'Japan',
      code: '+81',
      value: '81'
    },
    {
      country: 'Jersey',
      code: '+44 1534',
      value: '441534'
    },
    {
      country: 'Jordan',
      code: '+962',
      value: '962'
    },
    {
      country: 'Kazakhstan',
      code: '+7 6',
      value: '76'
    },
    {
      country: 'Kazakhstan',
      code: '+7 7',
      value: '77'
    },
    {
      country: 'Kenya',
      code: '+254',
      value: '254'
    },
    {
      country: 'Kiribati',
      code: '+686',
      value: '686'
    },
    {
      country: 'Korea, North',
      code: '+850',
      value: '850'
    },
    {
      country: 'Korea, South',
      code: '+82',
      value: '82'
    },
    {
      country: 'Kosovo',
      code: '+383',
      value: '383'
    },
    {
      country: 'Kuwait',
      code: '+965',
      value: '965'
    },
    {
      country: 'Kyrgyzstan',
      code: '+996',
      value: '996'
    },
    {
      country: 'Laos',
      code: '+856',
      value: '856'
    },
    {
      country: 'Latvia',
      code: '+371',
      value: '371'
    },
    {
      country: 'Lebanon',
      code: '+961',
      value: '961'
    },
    {
      country: 'Lesotho',
      code: '+266',
      value: '266'
    },
    {
      country: 'Liberia',
      code: '+231',
      value: '231'
    },
    {
      country: 'Libya',
      code: '+218',
      value: '218'
    },
    {
      country: 'Liechtenstein',
      code: '+423',
      value: '423'
    },
    {
      country: 'Lithuania',
      code: '+370',
      value: '370'
    },
    {
      country: 'Luxembourg',
      code: '+352',
      value: '352'
    },
    {
      country: 'Macau',
      code: '+853',
      value: '853'
    },
    {
      country: 'Madagascar',
      code: '+261',
      value: '261'
    },
    {
      country: 'Malawi',
      code: '+265',
      value: '265'
    },
    {
      country: 'Malaysia',
      code: '+60',
      value: '60'
    },
    {
      country: 'Maldives',
      code: '+960',
      value: '960'
    },
    {
      country: 'Mali',
      code: '+223',
      value: '223'
    },
    {
      country: 'Malta',
      code: '+356',
      value: '356'
    },
    {
      country: 'Marshall Islands',
      code: '+692',
      value: '692'
    },
    {
      country: 'Martinique',
      code: '+596',
      value: '596'
    },
    {
      country: 'Mauritania',
      code: '+222',
      value: '222'
    },
    {
      country: 'Mauritius',
      code: '+230',
      value: '230'
    },
    {
      country: 'Mayotte',
      code: '+262 269',
      value: '262269'
    },
    {
      country: 'Mayotte',
      code: '+262 639',
      value: '262639'
    },
    {
      country: 'Mexico',
      code: '+52',
      value: '52'
    },
    {
      country: 'Micronesia, Federated States of',
      code: '+691',
      value: '691'
    },
    {
      country: 'Midway Island, USA',
      code: '+1 808',
      value: '1808'
    },
    {
      country: 'Moldova',
      code: '+373',
      value: '373'
    },
    {
      country: 'Monaco',
      code: '+377',
      value: '377'
    },
    {
      country: 'Mongolia',
      code: '+976',
      value: '976'
    },
    {
      country: 'Montenegro',
      code: '+382',
      value: '382'
    },
    {
      country: 'Montserrat',
      code: '+1 664',
      value: '1664'
    },
    {
      country: 'Morocco',
      code: '+212',
      value: '212'
    },
    {
      country: 'Mozambique',
      code: '+258',
      value: '258'
    },
    {
      country: 'Myanmar',
      code: '+95',
      value: '95'
    },
    {
      country: 'Nagorno-Karabakh',
      code: '+374 47',
      value: '37447'
    },
    {
      country: 'Nagorno-Karabakh',
      code: '+374 97',
      value: '37497'
    },
    {
      country: 'Namibia',
      code: '+264',
      value: '264'
    },
    {
      country: 'Nauru',
      code: '+674',
      value: '674'
    },
    {
      country: 'Nepal',
      code: '+977',
      value: '977'
    },
    {
      country: 'Netherlands',
      code: '+31',
      value: '31'
    },
    {
      country: 'Nevis',
      code: '+1 869',
      value: '1869'
    },
    {
      country: 'New Caledonia',
      code: '+687',
      value: '687'
    },
    {
      country: 'New Zealand',
      code: '+64',
      value: '64'
    },
    {
      country: 'Nicaragua',
      code: '+505',
      value: '505'
    },
    {
      country: 'Niger',
      code: '+227',
      value: '227'
    },
    {
      country: 'Nigeria',
      code: '+234',
      value: '234'
    },
    {
      country: 'Niue',
      code: '+683',
      value: '683'
    },
    {
      country: 'Norfolk Island',
      code: '+672 3',
      value: '6723'
    },
    {
      country: 'North Macedonia',
      code: '+389',
      value: '389'
    },
    {
      country: 'Northern Cyprus',
      code: '+90 392',
      value: '90392'
    },
    {
      country: 'Northern Ireland',
      code: '+44 28',
      value: '4428'
    },
    {
      country: 'Northern Mariana Islands',
      code: '+1 670',
      value: '1670'
    },
    {
      country: 'Norway',
      code: '+47',
      value: '47'
    },
    {
      country: 'Oman',
      code: '+968',
      value: '968'
    },
    {
      country: 'Pakistan',
      code: '+92',
      value: '92'
    },
    {
      country: 'Palau',
      code: '+680',
      value: '680'
    },
    {
      country: 'Palestine, State of',
      code: '+970',
      value: '970'
    },
    {
      country: 'Panama',
      code: '+507',
      value: '507'
    },
    {
      country: 'Papua New Guinea',
      code: '+675',
      value: '675'
    },
    {
      country: 'Paraguay',
      code: '+595',
      value: '595'
    },
    {
      country: 'Peru',
      code: '+51',
      value: '51'
    },
    {
      country: 'Philippines',
      code: '+63',
      value: '63'
    },
    {
      country: 'Pitcairn Islands',
      code: '+64',
      value: '64'
    },
    {
      country: 'Poland',
      code: '+48',
      value: '48'
    },
    {
      country: 'Portugal',
      code: '+351',
      value: '351'
    },
    {
      country: 'Puerto Rico',
      code: '+1 787',
      value: '1787'
    },
    {
      country: 'Qatar',
      code: '+974',
      value: '974'
    },
    {
      country: 'Réunion',
      code: '+262',
      value: '262'
    },
    {
      country: 'Romania',
      code: '+40',
      value: '40'
    },
    {
      country: 'Russia',
      code: '+7',
      value: '7'
    },
    {
      country: 'Rwanda',
      code: '+250',
      value: '250'
    },
    {
      country: 'Saba',
      code: '+599 4',
      value: '5994'
    },
    {
      country: 'Saint Barthélemy',
      code: '+590',
      value: '590'
    },
    {
      country: 'Saint Helena',
      code: '+290',
      value: '290'
    },
    {
      country: 'Saint Kitts and Nevis',
      code: '+1 869',
      value: '1869'
    },
    {
      country: 'Saint Lucia',
      code: '+1 758',
      value: '1758'
    },
    {
      country: 'Saint Martin (France)',
      code: '+590',
      value: '590'
    },
    {
      country: 'Saint Pierre and Miquelon',
      code: '+508',
      value: '508'
    },
    {
      country: 'Saint Vincent and the Grenadines',
      code: '+1 784',
      value: '1784'
    },
    {
      country: 'Samoa',
      code: '+685',
      value: '685'
    },
    {
      country: 'San Marino',
      code: '+378',
      value: '378'
    },
    {
      country: 'São Tomé and Príncipe',
      code: '+239',
      value: '239'
    },
    {
      country: 'Saudi Arabia',
      code: '+966',
      value: '966'
    },
    {
      country: 'Senegal',
      code: '+221',
      value: '221'
    },
    {
      country: 'Serbia',
      code: '+381',
      value: '381'
    },
    {
      country: 'Seychelles',
      code: '+248',
      value: '248'
    },
    {
      country: 'Sierra Leone',
      code: '+232',
      value: '232'
    },
    {
      country: 'Singapore',
      code: '+65',
      value: '65'
    },
    {
      country: 'Sint Eustatius',
      code: '+599 3',
      value: '5993'
    },
    {
      country: 'Sint Maarten (Netherlands)',
      code: '+1 721',
      value: '1721'
    },
    {
      country: 'Slovakia',
      code: '+421',
      value: '421'
    },
    {
      country: 'Slovenia',
      code: '+386',
      value: '386'
    },
    {
      country: 'Solomon Islands',
      code: '+677',
      value: '677'
    },
    {
      country: 'Somalia',
      code: '+252',
      value: '252'
    },
    {
      country: 'South Africa',
      code: '+27',
      value: '27'
    },
    {
      country: 'South Georgia and the South Sandwich Islands',
      code: '+500',
      value: '500'
    },
    {
      country: 'South Ossetia',
      code: '+995 34',
      value: '99534'
    },
    {
      country: 'South Sudan',
      code: '+211',
      value: '211'
    },
    {
      country: 'Spain',
      code: '+34',
      value: '34'
    },
    {
      country: 'Sri Lanka',
      code: '+94',
      value: '94'
    },
    {
      country: 'Sudan',
      code: '+249',
      value: '249'
    },
    {
      country: 'Suriname',
      code: '+597',
      value: '597'
    },
    {
      country: 'Svalbard',
      code: '+47 79',
      value: '4779'
    },
    {
      country: 'Sweden',
      code: '+46',
      value: '46'
    },
    {
      country: 'Switzerland',
      code: '+41',
      value: '41'
    },
    {
      country: 'Syria',
      code: '+963',
      value: '963'
    },
    {
      country: 'Taiwan',
      code: '+886',
      value: '886'
    },
    {
      country: 'Tajikistan',
      code: '+992',
      value: '992'
    },
    {
      country: 'Tanzania',
      code: '+255',
      value: '255'
    },
    {
      country: 'Telecommunications for Disaster Relief by OCHA',
      code: '+888',
      value: '888'
    },
    {
      country: 'Thailand',
      code: '+66',
      value: '66'
    },
    {
      country: 'Togo',
      code: '+228',
      value: '228'
    },
    {
      country: 'Tokelau',
      code: '+690',
      value: '690'
    },
    {
      country: 'Tonga',
      code: '+676',
      value: '676'
    },
    {
      country: 'Transnistria',
      code: '+373 2',
      value: '3732'
    },
    {
      country: 'Trinidad and Tobago',
      code: '+1 868',
      value: '1868'
    },
    {
      country: 'Tristan da Cunha',
      code: '+290 8',
      value: '2908'
    },
    {
      country: 'Tunisia',
      code: '+216',
      value: '216'
    },
    {
      country: 'Turkey',
      code: '+90',
      value: '90'
    },
    {
      country: 'Turkmenistan',
      code: '+993',
      value: '993'
    },
    {
      country: 'Turks and Caicos Islands',
      code: '+1 649',
      value: '1649'
    },
    {
      country: 'Tuvalu',
      code: '+688',
      value: '688'
    },
    {
      country: 'Uganda',
      code: '+256',
      value: '256'
    },
    {
      country: 'Ukraine',
      code: '+380',
      value: '380'
    },
    {
      country: 'United Arab Emirates',
      code: '+971',
      value: '971'
    },
    {
      country: 'United Kingdom',
      code: '+44',
      value: '44'
    },
    {
      country: 'United States',
      code: '+1',
      value: '1'
    },
    {
      country: 'Universal Personal Telecommunications (UPT)',
      code: '+878',
      value: '878'
    },
    {
      country: 'Uruguay',
      code: '+598',
      value: '598'
    },
    {
      country: 'US Virgin Islands',
      code: '+1 340',
      value: '1340'
    },
    {
      country: 'Uzbekistan',
      code: '+998',
      value: '998'
    },
    {
      country: 'Vanuatu',
      code: '+678',
      value: '678'
    },
    {
      country: 'Venezuela',
      code: '+58',
      value: '58'
    },
    {
      country: 'Vietnam',
      code: '+84',
      value: '84'
    },
    {
      country: 'Wake Island, USA',
      code: '+1 808',
      value: '1808'
    },
    {
      country: 'Wallis and Futuna',
      code: '+681',
      value: '681'
    },
    {
      country: 'Yemen',
      code: '+967',
      value: '967'
    },
    {
      country: 'Zambia',
      code: '+260',
      value: '260'
    },
    {
      country: 'Zanzibar',
      code: '+255 24',
      value: '25524'
    },
    {
      country: 'Zimbabwe',
      code: '+263',
      value: '263'
    }
  ];

  constructor() {
  }

  ngOnInit() {
  }

  onSelectCountryCode($event) {
    const value = $event.target.value;
    this.countrySelectHandler.emit(value ? JSON.parse(value) : null);
  }

  stringify(item) {
    return JSON.stringify(item);
  }

}
